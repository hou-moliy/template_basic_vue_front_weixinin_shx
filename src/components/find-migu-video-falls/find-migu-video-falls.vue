<template>
  <view class="more-news" :style="{ marginTop }">
      ...
      <view class="waterfall-column waterfall-column-left">
        <block v-for="(item, index) in miguVideoList" :key="index">
          <view v-if="index % 2 === 0" class="more-new-item-column">
            <!-- 运营位 -->
            <view class="recommend-wrap" v-if="item.sourceType == 2">
              <operate-item
                :item="item"
    ...
    <uni-load-more
      ...
      :status="isLoadStatus"
    />
    <!-- <uni-load-more v-else="" class="loadingicon" iconSize="20" iconType="circle" :status="isLoadStatus" /> -->
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    // 滚动触底
    onScollBottom() {
      let curCount = this.miguVideoList.length;
      if (curCount >= this.total) {
        // 没有数据了
        // this.$store.commit("miguVideo/SET_LOAD_STATUS", "nomore");
        return;
      }
      // this.$store.commit("miguVideo/SET_LOAD_STATUS", "loading");
      ...
      miguService.getlistById(wfParams).then(({ data: res }) => {
        ...
        if (res.code === 200) {
          ...
          if (flag) {
            ...
            let oldLen = this.miguVideoList.length;
