<template>
	<view :class='["uni-popup-dialog",isOpen?"uni-popup-dialog-open":""]'>
		...
		<view class='uni-dialog-top' :style="{height:isOpen?'308rpx': '236rpx' }">
			...
			<view :class='["uni-dialog-content",isOpen?"uni-dialog-content-open":""]'>
				...
				<text :class='["uni-dialog-content-text",isOpen?"uni-dialog-content-text-open":""]' v-else-if="mode === 'open'">音频彩铃业务标准资费<text
					 style="color: #5E4827;font-weight: bold;"> 5元/月</text> ,是否马上开通？开通彩铃畅享海量彩铃音乐，快来挑选你喜欢的音乐作为彩铃吧！</text>
				<input v-else class="uni-dialog-input" v-model="val" type="text" :placeholder="placeholder" :focus="focus">
			</view>
			<view class="uni-dialog-check" v-if="isOpen">
				<view @click="check" class="list-checkbox" :class="checked?'list-checked':''">
					<text v-if="checked" class="iconfont icon-xuanzhong"></text>
				</view>
				<view class='check-view-conten-text'>已阅读并同意<view style="color: #3FC2FC;display: inline-block;" @click="navicateToMusicOrderPolicy">《订购协议》</view>
				</view>
			</view>
		</view>
		...
		<view class="uni-dialog-button-group">
			...
			<view class="uni-dialog-button uni-border-left" @click="onOk">
				<text :class="['uni-dialog-button-text uni-button-color' ,isOpen?'uni-dialog-content-ok-text-open':'']">{{confirmConent}}</text>
			</view>
		</view>
