	...
	<view class="login-page">
		<view class="top-box">
			...
			<view class="input-group">
				...
				<view class="sms_login">
					<!-- <button class="login-btn" size="mini" open-type="getUserInfo" @getuserinfo="getUserInfo">立即登录</button> -->
		...
		<view class="bottom-box">
			<view class="wx-login-box">
				...
				<!-- 微信登录提取 -->
				<!-- 	<view v-if="!ischecked&isShowWx" @click="noIschecked">
									<image class="wx-login-img" :src="`${staticImgs}/lnmp/yjdl/freeLogin-wx.png`"></image>
								</view>
								<button v-else @getphonenumber="wxLogin($event, 'wx')" open-type="getPhoneNumber" v-show="isShowWx">
									<image class="wx-login-img" :src="`${staticImgs}/lnmp/yjdl/freeLogin-wx.png`"></image>
								</button> -->
				<!-- 微信end -->
			</view>
	...
	export default {
		...
		methods: {
			...
			sendCode() {
				...
				/*  const param = {
			      ...
			      if (this.sendFlag) {
			        ...
			        axios.get(`${this.portalUrl}/sso/getAuthCodeLn`, param).then((resp) => {
			          this.sendFlag = false
			          uni.hideLoading()
			          if (resp.status === 200) {
			            this.sendFlag = true
			            if (resp.data.code === 506) {
			              ...
			              return uni.showToast({
			                title: '您不是辽宁移动手机号码，请使用辽宁移动手机号登录',
			                icon: 'none',
			                duration: 2000,
			              })
			            } else if (resp.data.code !== 200) {
			              return this.showToast(resp.data.message)
			            }
			...
			getUserProfile(e) {
				...
				if (!this.password && !this.isSimLogin) {
					return uni.showToast({
						title: '请输入验证码',
						icon: 'none',
						duration: 2000,
			...
			successLogin(res) {
				...
				if (res.data.code === 506) {
					uni.hideLoading()
					return uni.showToast({
						title: '您不是辽宁移动用户,请输入辽宁移动手机号码完成登录',
						icon: 'none',
						duration: 2000,
					})
