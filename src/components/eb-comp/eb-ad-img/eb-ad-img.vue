  ...
  methods: {
    ...
    navigateToH5 (event) {
      if (event.eventUrl === "/pagesSpcl/clVideo/clVideoSearch") { // 视频搜索埋点
        this.$analysis.dispatch("video_search_count", this.activityId).finally(() => navigateToAnyCheck(event, `video_search_count_${event.id}`));
      } else {
        this.$analysis.dispatch("ad_img", `${this.activityId}_${event.id}`).finally(() => navigateToAnyCheck(event, `ad_img_${event.id}`));
      }
    },
  },
};
